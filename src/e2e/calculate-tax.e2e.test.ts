...
describe('E2E Tests', () => {
	...
	test(
		'should submit input and display result',
		async () => {
			...
			const optionToSelect = await dropdown.findElement(
				By.css(`option[value="${formInputs.year}"]`),
			);
			...
			const isVisible = await driver
				.wait(until.elementIsVisible(element), 10000)
				.then(() => true)
