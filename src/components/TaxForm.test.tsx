...
describe('TaxForm Component', () => {
	it('should dispatch action on valid form submission', async () => {
		...
		fireEvent.change(getByLabelText(/annual income/i), {
			target: { value: formInputs.income },
		});
		fireEvent.change(getByLabelText(/tax year/i), {
			target: { value: formInputs.year },
		});
	...
	it('should show validation error for invalid annual income', async () => {
		...
		expect(
			await getByText(/annual income must be a positive number/i),
		).toBeInTheDocument();
	});
	...
	it('should show validation error when tax year is not selected', async () => {
		...
		expect(
			await getByText(/please select a tax year/i),
		).toBeInTheDocument();
	});
