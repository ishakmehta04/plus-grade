...
export function* calculateSaga({
	payload: { income, year },
}: PayloadAction<CalucatedTaxPayload>) {
	try {
		// You can also export the axios call as a function.
		const response: AxiosResponse<TaxApiResponseType> = yield axios.get(
			`${process.env.REACT_APP_API_URL}/tax-calculator/tax-year/${year}`,
		);
